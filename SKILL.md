...
2. Script path = `{baseDir}/scripts/<script-name>.py`
3. Use `python3` to execute scripts

| Script | Purpose |
|--------|---------|
| `scripts/merge-to-pptx.py` | Merge slides into PowerPoint |
| `scripts/merge-to-pdf.py` | Merge slides into PDF |
...
**Standard flow**:
1. Create `slides/` directory: `mkdir -p {baseDir}/slide-deck/{topic-slug}/slides`
2. Generate session ID: `slides-{topic-slug}-{timestamp}`
3. For each slide (with concurrency control):
   - Read prompt content from `prompts/NN-slide-{slug}.md`
   - Extract simplified prompt (remove markdown formatting, keep essential visual requirements)
   - Submit generation task using shortart-image-generator text-to-image mode
   - Use `--model nano-banana-pro --aspect-ratio 16:9 --resolution 2k`
   - Run tasks in background (max 10 concurrent)
4. Wait for all tasks to complete
5. Download images:
   - Use `cd ~/.claude/skills/shortart-image-generator/scripts/text-to-image && python3 impl.py --poll {project_id}`
   - Extract image URL from response
   - Download with `curl -o {baseDir}/slide-deck/{topic-slug}/slides/NN-slide-{slug}.png {url}`
6. Report progress: "Generated X/N" (in user's language)

**Concurrency Control**:
- Submit up to 10 image generation tasks concurrently
- Wait for batch completion before downloading
- Maintain session ID consistency across all batches

**Prerequisites**:
- `shortart-image-generator` skill must be installed
- `SHORTART_API_KEY` environment variable must be configured (get from https://shortart.ai/home)
...
**Note**: Scripts will read slide images from `slides/` directory.

**Prerequisites**:
- Python 3 with `python-pptx` and `Pillow` packages installed
...
- slides/01-slide-cover.png - Cover
- slides/02-slide-intro.png - Content
- ...
- slides/{NN}-slide-back-cover.png - Back Cover
...
2. Generate image using same session ID to `slides/` directory
...
1. Remove `slides/NN-slide-{slug}.png` and `prompts/NN-slide-{slug}.md`
